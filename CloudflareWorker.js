...
async function makeStreamRequest(request) {
  ...
  // doesn't work
  let response = await fetch(request);
  const content = (await response.text()).replaceAll(/https:\/\/([^.]+).strm.yandex.net/gi, ''); // replacing yandex balancer with emptiness. It may be fixed soon, but it's still working
...
async function handleStreamProxyRequest(audioName, search) {
  // TODO: remove
  const audioRequest = new Request('https://strm.yandex.ru/' + audioName + search, {
    headers: {
      "User-Agent": yandexUserAgent,
...
addEventListener('fetch', event => {
  ...
  } else if (url.pathname.startsWith('/transcoderless/') && (url.pathname.endsWith('.m3u8') || url.pathname.endsWith(".ts"))) {
    // proxy m3u8 endpoint
    if (request.method !== 'GET')
      return event.respondWith(errorResponse('error-method'));

    return event.respondWith(handleStreamProxyRequest(url.pathname, url.search));
  } else {
