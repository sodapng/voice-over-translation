# 1.5.0-beta3
- Добавлена эксперементальная поддержка трансляций (Необходимо расширение для отключение CORS. С задержкой как повезет, но в большинстве случаев довольно терпимо +-0-2 секунды от оригинала)
- Добавлен HLS.js
