...
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   a: () => (/* binding */ autoVolume),
/* harmony export */   b: () => (/* binding */ yandexUserAgent)
/* harmony export */ });
...
async function requestVideoTranslation(
  ...
  const body = yandexRequests/* yandexRequests */.a.encodeRequest(
    url,
    deviceId,
    unknown1,
    requestLang,
    ...
    const options = {
      ...
      body: JSON.stringify({
        headers: {
          ...
          "User-Agent": config/* yandexUserAgent */.b,
          Pragma: "no-cache",
...
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   a: () => (/* binding */ yandexRequests)
/* harmony export */ });
...
function addTranslationBlock(element) {
  ...
  debug/* default */.a.log("VOT: Added translation button to ", element);
}
...
async function src_main() {
  ...
  function addTranslationMenu(element) {
    ...
    debug/* default */.a.log("VOT: Added translation menu to ", element);
  }
  ...
  function translateVideo(url, unknown1, requestLang, responseLang, callback) {
    ...
    requestVideoTranslation(
      ...
      (success, response) => {
        ...
        const translateResponse = yandexRequests/* yandexRequests */.a.decodeResponse(response);
  ...
  async function translateProccessor(videoContainer, siteHostname, siteEvent) {
    debug/* default */.a.log("[translateProccessor] execute ", videoContainer);
    ...
    debug/* default */.a.log("videoContainer", videoContainer);
    ...
    debug/* default */.a.log("video", video);
    ...
    if (isDBInited) {
      ...
      if (dbData) {
        ...
        debug/* default */.a.log("[db] data from db: ", dbData);
        ...
        if (
          ...
          menuOptions
            ...
            .addEventListener("change", (event) => {
              debug/* default */.a.log("[onchange] select from language", event.target.value);
          ...
          menuOptions
            ...
            .addEventListener("change", (event) => {
              debug/* default */.a.log("[onchange] select to language", event.target.value);
        ...
        if (
          ...
          const checkbox = createMenuCheckbox(
            ...
            `Уменьшать громкость видео до ${config/* autoVolume */.a * 100}%`
          );
        ...
        if (
          ...
          checkbox.querySelector("#VOTSyncVolume").onclick = async (event) => {
            ...
            dbSyncVolume = value;
            debug/* default */.a.log("syncVolume value changed. New value: ", dbSyncVolume);
          };
    ...
    const lipSync = (mode = false) => {
      debug/* default */.a.log("lipsync video", video);
      if (!video) {
        return;
      }
      ...
      if (mode === "pause") {
        debug/* default */.a.log("lipsync mode is pause");
        audio.pause();
      }
    };
    ...
    function videoValidator() {
      ...
      if (window.location.hostname.includes("youtube.com")) {
        let ytData = getYTVideoData();
        ytData = setDetectedLangauge(ytData, ytData.detectedLanguage);
        debug/* default */.a.log("VideoValidator videoData: ", videoData);
    ...
    function translateFunc(VIDEO_ID, requestLang, responseLang) {
      ...
      translateVideo(
        ...
        responseLang,
        (success, urlOrError) => {
          debug/* default */.a.log("[exec callback] translateVideo");
          ...
          if (
            ...
            video.volume = config/* autoVolume */.a;
          }
          ...
          if (VOTVideoSlider) VOTVideoSlider.value = config/* autoVolume */.a * 100;

          const VOTOriginalVolume =
            document.querySelector("#VOTOriginalVolume");
          if (VOTOriginalVolume) {
            VOTOriginalVolume.innerText = `${config/* autoVolume */.a * 100}%`;
          }
    ...
    document.addEventListener("click", (event) => {
      ...
      debug/* default */.a.log(`[document click] ${isBlock} ${isContent} ${isVideo}`);
    ...
    document
      ...
      .addEventListener("click", async (event) => {
        debug/* default */.a.log("[click translationBtn] before all functions & methods");
        event.stopPropagation();
  ...
  async function initWebsite() {
    if (config_regexes.youtubeRegex.test(window.location.hostname)) {
      debug/* default */.a.log("[entered] YT Regex Passed", config_regexes.youtubeRegex);
      const ytPageEnter = () => {
        const videoContainer = document.querySelector(
          config_selectors.youtubeSelector
        );
        if (videoContainer) {
          debug/* default */.a.log("[exec] translateProccessor youtube on page enter");
          translateProccessor(videoContainer, "youtube", "yt-translate-stop");
        } else {
          if (!ytplayer || !ytplayer.config) {
            debug/* default */.a.log("[exec] ytplayer is null");
            return;
          }
      ...
      if (window.location.hostname.includes("twitch.tv")) {
        debug/* default */.a.log("[entered] Twitch");
        if (
          window.location.hostname.includes("m.twitch.tv") &&
          (window.location.pathname.includes("/videos/") ||
            window.location.pathname.includes("/clip/"))
        ) {
          debug/* default */.a.log("[entered] twitch mobile");
    ...
    if (window.location.hostname.includes("xvideos.com")) {
      debug/* default */.a.log("[entered] xvideos");
      await sleep(1000);
      await translateProccessor(
    ...
    if (window.location.hostname.includes("pornhub.com")) {
      debug/* default */.a.log("[entered] pornhub");
      await sleep(1000);
      await translateProccessor(
    ...
    if (sitesInvidious.includes(window.location.hostname)) {
      ...
      debug/* default */.a.log("[entered] invidious");
      await translateProccessor(
      ...
      // Need an additional extension to work in chrome-like browsers
      debug/* default */.a.log("[entered] piped");
      ...
      debug/* default */.a.log("[entered] vk.com");
      ...
      debug/* default */.a.log("[entered] vimeo.com");
