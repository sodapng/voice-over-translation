...
async function requestVideoTranslation(url, unknown1, requestLang, responseLang, callback) {
  ...
  try {
    ...
    const key = await window.crypto.subtle.importKey('raw', utf8Encoder.encode(config/* yandexHmacKey */.I1), { name: 'HMAC', hash: {name: 'SHA-256'}}, false, ['sign', 'verify']);
      ...
      if (dbData) {
        ...
        if (dbShowVideoSlider !== undefined && menuOptions && !menuOptions.querySelector('#VOTShowVideoSlider')) {
          ...
          checkbox.querySelector('#VOTShowVideoSlider').onclick = async (event) => {
            ...
            utils_debug.log('showVideoSlider value changed. New value: ', dbShowVideoSlider);
      ...
      const ytPageEnter = (event) => {
        ...
        if (videoContainer != null) {
          ...
          ytplayer.config.args.jsapicallback = (jsApi) => {
            utils_debug.log('[exec] translateProccessor youtube on page enter (ytplayer.config.args.jsapicallback)')
            translateProccessor(videoContainer, 'youtube', 'yt-translate-stop');
          }
        }
      };
