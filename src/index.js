  ...
  async function translateProccessor(videoContainer, siteHostname, siteEvent) {
    ...
    async function updateSubtitlesLangSelect() {
      ...
      for (let i = 0; i < subtitlesList.length; i++) {
        ...
        option.value = i;
        option.innerHTML = 
          (translations[lang][availableLangs[s.language]] ?? s.language) +
          (s.translatedFromLanguage ? 
            ` translated from ${translations[lang][availableLangs[s.translatedFromLanguage]] ?? s.translatedFromLanguage}` : "") +
