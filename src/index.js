...
async function main () {
      ...
      (success, response) => {
        ...
        switch (translateResponse.status) {
          ...
          case 1:
            ...
            callback(
              hasUrl,
              hasUrl ? translateResponse.url : 'Не получена ссылка на аудио'
            )
            return
          case 2:
            const remainingTime = translateResponse.remainingTime
  ...
  async function translateProccessor (videoContainer, siteHostname, siteEvent) {
    ...
    function stopTraslate () {
      ...
      document
        .querySelector('video')
        .removeEventListener('.translate', stopTraslate, false)
      deleteAudioSrc()
      document.querySelector('#VOTVideoSlider')?.parentElement.remove()
            ...
            for (const mutation of mutations) {
              if (
                ...
                const video = mutation.target
                  .closest(selectors.twitchMobileSelector)
                  ?.querySelector('video')
                if (video) {
                  await sleep(1000)
