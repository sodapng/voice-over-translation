...
app.post(
  ...
  async (req, res) => {
    logger.info(
      `Получено новое видео на перевод (https://www.youtube.com/watch?v=${req.body.video_id})`
    );
    try {
    ...
    if (page !== null && page !== undefined) {
      try {
        ...
        try {
          await page.evaluate(() => {
            if (document.getElementsByClassName("ytp-ad-skip-button")) {
              ...
              if (skip !== undefined) {
                skip.click();
                logger.debug(
                  `Реклама была пропущена (https://www.youtube.com/watch?v=${req.body.video_id})`
                );
              }
            }
          });
          ...
          logger.debug(
            `На странице не найдена реклама (https://www.youtube.com/watch?v=${req.body.video_id})`
          );
        }
        ...
        try {
          ...
          logger.error(
            `Не удалось перевести видео (https://www.youtube.com/watch?v=${req.body.video_id})`
          );
          ...
          try {
            ...
            await page.close();
            logger.info(
              `Страница закрыта из-за невозможности перевода (https://www.youtube.com/watch?v=${req.body.video_id})`
            );
            return;
          } catch (err) {
            createBrowser();
      ...
      page.on("request", async (request) => {
        if (request.url().includes(".mp3")) {
          try {
            ...
            try {
              logger.info(
                `Видео было переведено (https://www.youtube.com/watch?v=${req.body.video_id})`
              );
            } catch {}
          } catch (err) {
            logger.error(err.message);
      ...
      return logger.info(
        "Браузер был перезапущен. Нажмите на кнопку перевода ещё один раз"
      );
    }
  }
);
